...
export class PEPipe extends Pipe
{
    ...
    constructor(od : string | Quantity, pn : string)
    {
		od = new Quantity(od);
		pn = ""+pn;

		let odStr = od.to("mm").scalar.toString();
		...
		let odData = PEPipe.WT_DATA[odStr];
		...
		let wt = new Quantity(odData[pn], "mm");
